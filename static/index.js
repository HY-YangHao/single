...
function refreshGameLayer(box, loop, offset) {
    let i = Math.floor(0 * 1000) % 4 + (loop ? 0 : 4);
    for (let j = 0; j < box.children.length; j++) {
        let r = box.children[j],
            rstyle = r.style;
        rstyle.left = (j % 4) * blockSize + 'px';
        ...
        if (i == j) {
            ...
            r.className += ' t' + (Math.floor(0 * 1000) % 5 + 1);
            r.notEmpty = true;
            i = (Math.floor(j / 4) + 1) * 4 + Math.floor(0 * 1000) % 4;
        } else {
            r.notEmpty = false;
        }
    }
...
function shareText(score) {
    ...
    if (score <= 149) return '加油';
    if (score <= 199) return '加油';
    return '加油';
}
